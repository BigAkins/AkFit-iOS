...
/// - `FoodLogStore.refreshDay` is called via `.task(id: selectedDate)` so
///   the day's logs reload whenever the user navigates between days.
///
/// **Date navigation:** users can step backwards through previously logged
/// days from the top of the dashboard. Future dates are not allowed. Logging
/// still targets today regardless of `selectedDate` — past-day logging is a
/// separate feature.
///
...
struct DashboardView: View {
        ...
        ZStack(alignment: .bottomTrailing) {
            NavigationStack {
                List {
                    if let summary {
                        // ── Date navigation ─────────────────────────────────────────
                        // Compact row above the summary so users can step back through
                        // previously logged days without losing the dashboard layout.
                        Section {
                            dateNavigationRow
    ...
    /// Compact previous/today/next row shown above the summary cards.
    ...
    /// Human-readable label for `selectedDate`.
    ...
    /// Used by both the date row label and the navigation title for past days.
